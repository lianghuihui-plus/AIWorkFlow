...
需求和任务规划 revision 继续使用稳定 ID 与局部补丁。accepted 需求集合增减时，design 和 task-plan 进入核对；行为变化只影响 design 和直接引用该需求的任务产物。实现修订只影响自身测试和一层直接依赖任务的实现、测试，不递归传播；无关任务保持可执行。核对后无需修改时通过 `reconcile` 清除标记，不重新审核未变内容。
